...
import cn.stylefeng.guns.modular.busi.mapper.*;
...
public class ConstantFactory implements IConstantFactory {
    ...
    //add by zhuhailin
    private CustomerinfoMapper customerinfoMapper = SpringContextHolder.getBean(CustomerinfoMapper.class);

    //zhangqitao
    private WatermeterinfoMapper watermeterinfoMapper = SpringContextHolder.getBean(WatermeterinfoMapper.class);
    ...
    @Override
    @Cacheable(value=Cache.CONSTANT,key = "'" +CacheKey.CUSTOMERINFO_NAME+"'+#customerinfoId")
    public String getCustomerinfoName(Long customerinfoId)
    {
        if(customerinfoId == null)
            return "";
        else {
            Customerinfo cus = customerinfoMapper.selectById(customerinfoId);
            if(ToolUtil.isNotEmpty(cus) && ToolUtil.isNotEmpty(cus.getCustomername()))
            {
    ...
    @Override
    public String getWaterMeterDeviceId(Long watermeterinfoId) {
        if(watermeterinfoId == null)
            return "";
        else {
            Watermeterinfo wat = watermeterinfoMapper.selectById(watermeterinfoId);
            if(ToolUtil.isEmpty(wat) && ToolUtil.isNotEmpty(wat.getDeviceid())){
                return wat.getDeviceid();
            }
        }
        return null;
    }
